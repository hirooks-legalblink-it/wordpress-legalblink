...
export class BaseApiService {
  ...
  constructor (config?: Partial<ApiConfig>) {
    this.config = {
      root: window.wplb.root,
      nonce: window.wplb.nonce,
      timeout: 30_000,
      ...config,
    }
  ...
  private async request<T = any>(
    ...
    // a query string (e.g. `https://site.com/?rest_route=/wplb/v1`). Joining
    // the endpoint with `/` keeps the REST route inside that query value
    // (`?rest_route=/wplb/v1/banner`), but the extra params must be appended
