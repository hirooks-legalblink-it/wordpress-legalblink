...
if (!class_exists('WPLB_Document_API_Controller')) {
    /**
     * Document API Controller
     * Handles document-related API requests to external service
     */
    class WPLB_Document_API_Controller extends WPLB_Base_API_Controller
    {
        /**
        ...
        {
            try {
                $cached_documents = WPLB_Transient_Helper::getCachedApiResponse('documents');
                if ($cached_documents !== false) {
                    WPLB_Logger::info('Documents retrieved from cache', WPLB_Logger::CATEGORY_API, 'get_documents');
                    foreach ($cached_documents as $policy_type => &$document) {
                        foreach ($document['languages'] as $iso_code => &$language) {
                            $page_id = WPLB_Option_Helper::getLanguageOption("page_{$policy_type}_id", $iso_code, '0');
                            $use_html_snippet = WPLB_Option_Helper::getLanguageOption("page_{$policy_type}_use_html_snippet", $iso_code, false);
                ...
                WPLB_Transient_Helper::cacheApiResponse('documents', $parsed_documents);

                // We store urls in options for quick access
                foreach ($parsed_documents as $policy_type => &$document) {
                    foreach ($document['languages'] as $iso_code => &$language) {
                        WPLB_Option_Helper::setLanguageOption("documents_{$policy_type}_html_url", $language['url']['html'], $iso_code);
                        $page_id = WPLB_Option_Helper::getLanguageOption("page_{$policy_type}_id", $iso_code, '0');
                        $use_html_snippet = WPLB_Option_Helper::getLanguageOption("page_{$policy_type}_use_html_snippet", $iso_code, false);
                        $language['pageId'] = $page_id === '0' ? null : $page_id;
                        $language['useHtmlSnippet'] = (bool)$use_html_snippet;
                ...
                WPLB_Logger::info('Documents retrieved successfully: ' . wp_json_encode($documents_response), WPLB_Logger::CATEGORY_API, 'get_documents');
                return $this->create_api_response(true, $documents_response);
            } catch (Exception $e) {
        ...
        public function update_page_content($request)
        {
            try {
                ...
                WPLB_Option_Helper::setLanguageOption("page_{$policy_type}_id", $page_id, $language);
                WPLB_Option_Helper::setLanguageOption("page_{$policy_type}_use_html_snippet", $use_html_snippet, $language);
